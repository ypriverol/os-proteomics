...
Research software is essential to modern science, with the majority of scientists recognizing it as indispensable for their work and indispensible for their research [28751965, https://zenodo.org/records/14809].
...
Since the original publication of the first mass spectral database search tool, SEQUEST [24226387], proteomics software has evolved into a sophisticated ecosystem encompassing multiple stages of data processing, advanced predictive models, and robust computational frameworks.
...
Numerous open source software tools have been developed and used by the proteomics community. These include tools like Percolator [17952086], which is used to improve peptide and protein identification using machine learning, Prosit [31133760], which applies deep learning to predict fragment ion intensities, aiding in more accurate spectra matching, and Proteowizard [23051804], which provides shared libraries and tools for data access. Platforms like GalaxyP [29092937] and quantms [38965444] facilitate accessible, reproducible analyses through high-performance computing and distributed workflows, supporting researchers in handling large datasets and complex analyses.
Together, these advances underscore how proteomics software has transformed into a multidisciplinary field, involving a complex ecosystem of algorithms, models, and software tools requiring sophisticated computational and algorithmic expertise.
...
- Complex workflows and high-throughput data analysis are growing in proteomics [29092937, 38965444, 36648445]. This complexity requires managing dependencies, configurations, and environments consistently across diverse systems and architecture without human intervention.
 The community has tackled this challenge with [Spell out acronyms --WSN] CI/CD pipelines as, for example, described in 10.1101/gr.276963.122, but this relies on OSS [This abbreviation is not introduced till later. --WSN] and the permission to freely redistribute software along the entire dependency chain.
 If one piece in this supply chain is not OSS, exceptions need to be handled, and automation is harder or impossible.
 In sum, it [what does "it" refer to? --WSN] creates a huge additional burden for the entire community downstream of the non-OSS software package, which on its own is a hidden cost.
...
- **Long-term Reliability**: Because the code is publicly accessible, open-source projects are less dependent on single organizations or developers, promoting continuity and stability even if the original contributors leave.
...
- **Obscure Dependencies**: Software repositories may have dependencies that are not clearly documented, which may require closed-source or proprietary software. This can create barriers for other researchers attempting to run or build on the software, as they may not have access to necessary components or may need to purchase expensive licenses. Clear documentation of all dependencies along with their licensing terms is essential to ensure transparency and reproducibility.
...
As the gold standard for proteomics software development, we recommend using a standard OSS license like Apache 2.0, MIT, BSD, LGPL, GPL, etc.; a list of them can be found at (https://opensource.org/licenses).
...
Many proteomics code repositories do not have a software license specified (**Figure 1**).
...
The fact that so many proteomics tools have unspecified licenses suggests a misunderstanding of software licensing.
...
It is recommended to clearly state the dependencies that a piece of software might have and the licenses of each of them. Full disclosure of such dependencies is necessary to ensure that the user is aware of this, such that the community, developers, and journal reviewers are able to understand this challenge.
...
Many other funding agencies have similar open source guidelines and mandates.
...
- **Potential Solutions**:
  - **Dedicated Maintenance Grants**: Funding agencies should create grant mechanisms for software maintenance, such as the Chan—Zuckerberg Initiative’s "Essential Open Source Software for Science" grants, and expand these options to the NIH.  [These used to exist.  Cf. https://grants.nih.gov/grants/guide/pa-files/PAR-08-010.html   Should also mention https://grants.nih.gov/grants/guide/rfa-files/RFA-OD-24-010.html  ---WSN]
...
A piece of software being open-source does not necessarily mean that it does not have costs associated with maintaining, running and developing the software. Because of this, strategies to commercialize OSS have been developed to ensure that the software can be maintained and developed in the long term.
