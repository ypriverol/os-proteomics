...
- Complex workflows and hight-trouble data analysis now depend heavily on unattended software deployment. This process is challenging, as it requires managing dependencies, configurations, and environments consistently across diverse systems and architectures without human intervention. The community has addressed this with CI/CD pipelines, as described in [36828587], which rely on open-source software and the freedom to redistribute it throughout the entire dependency chain. However, if any component in this chain is not open-source, exceptions must be handled, making automation difficult or even impossible. This creates a significant downstream burden on the community, adding hidden costs tied to the non-open-source software.
