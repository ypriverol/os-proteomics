...
- Custom licenses and restrictions on software distribution can further complicate the situation, making it difficult to share, modify, or redistribute software, and hindering the development of a collaborative and open-source ecosystem.
 Proteomics software is often distributed under restrictive licenses and tailored to specific platforms (e.g., operating systems, computer architectures), limiting its use across diverse environments and services. Not surprisingly, such restrictive licenses limit the field’s adaptability and hinder the integration of proteomics with other omics disciplines.

- Complex workflows are becoming the norm in proteomics [29092937, 38965444, 36648445], making it difficult to manage dependencies, configurations, and environments consistently across diverse systems and architecture without human intervention.
 ...
 If one piece in this supply chain is not OSS, exceptions need to be handled, and automation becomes much harder or even impossible.
 A substantial additional burden is therefore created downstream of any non-OSS software package, which is a hidden cost on its own, and one that affects the entire community.

- The lack of standardization in proteomics software development, including inconsistent documentation, variable code quality, and limited community engagement, can hinder the adoption and use of software tools, even if OSS, leading to inefficiencies and redundancies in software development.
...
Crucially, until recently, major instrument vendors lacked open-source, cross-platform libraries for data access, limiting data reuse and algorithm development [16041670].
Thermo Fisher's RawFileReader library marks important progress in this respect, enabling tools like ThermoRawFileParser [31755270] and PRIDE Archive USI [39494541,34183830]. This idea has recently been extended for Bruker timsTOF data with the timsrust library (https://github.com/MannLabs/timsrust/), which is open-source and already used by tools like the Sage search engine [37819886].

A solution to many of these challenges is offered by open-source software (OSS) practices, which are also aligned with the FAIR Principles (Findable, Accessible, Interoperable, Reusable) that have initially been established for scientific data [26978244]. FAIR principles were expanded in 2022 to research software (FAIR4RS) to address the growing recognition of research software as a foundational research asset [36241754]. Adhering to FAIR4RS principles empowers proteomics with OSS tools that are not only accessible but that also foster community-driven development, rigorous validation, and transparent sharing of methodologies [37768885, 36241754]. 
Although OSS is not an explicit requirement for implementing FAIR principles, it greatly facilitates the realization of these principles by making software more accessible, more transparent, and much more easily reusable. 
Indeed, OSS has already demonstrated clear benefits in increasing the accessibility, usability, and visibility of scientific software [30212065].
With code freely available for inspection, modification, and distribution, OSS encourages collaboration and creates avenues for continuous improvement, reproducibility, traceability and auditability; all factors that are critical in fields as data-intensive as proteomics.
...
Our goal is therefore to present a vision for a future where OSS and FAIR software are even more strongly encouraged and supported in the proteomics community.
...
- **Transparency and Community Trust**: With open-source, the code is transparent by design, allowing the community to understand, verify, and contribute to the project. This fosters trust and credibility, especially crucial characteristics in scientific fields.
...
- **Long-term Reliability**: Because the code is publicly accessible, open-source projects are less dependent on single organizations or developers for their maintenance and long-term survival, promoting continuity and stability even if original contributors leave.

- **No Restriction to Specific User Groups**: Unlike “free-for-academic-use” licenses, which restrict usage to academic settings, open-source licenses do not impose limitations on the types of users or institutions that can access or use the software. Moreover, many OSI licenses (most famously the Apache 2.0 license (https://opensource.org/license/apache-2-0)) are explicitly compatible with commercial purposes, also in derived works.

- **Not Necessarily Free of Cost**: Open-source software is “free” in terms of freedom, not necessarily in terms of price. Users might pay for support, hosting, or additional services, even for the compiled software itself, but they retain freedom in how they use and modify the source code of the software.
...
- Cost-free software is not always open-source. Many programs are freely available for non-commercial or academic use but do not meet open-source criteria. Similarly, "free and open-source software" (FOSS), a combined moniker for 'free software' on the one hand, and OSS on the other hand, refers to the freedom to run, modify, and share the software, not necessarily its financial cost. FOSS may involve expenses for services like support or hosting, but it ensures users retain the freedom to use, adapt, and distribute the software as they wish (https://www.gnu.org/philosophy/free-sw.html). The nuanced distinction between 'free software' and OSS is that the former is strongly oriented towards the need to keep derived works free as well (most notable in the GNU GPL licence (https://opensource.org/license/gpl-3-0)), whereas OSS typically allows derived works to retain (partially) closed code as well (see, for instance, the abovementioned Apache 2.0 license).
- Academic licenses only refer to free-for-academic-use. The source code is not necessarily open, shareable, or modifiable. Moreover, the term "academic" is not well-defined as it can refer to a wide range of institutions and organizations. By contrast, OSS is defined as any software that uses a license approved by the Open Source Initiative (OSI, https://opensource.org/licenses).
- Accessible source code does not mean open-source. Open-source software does not only mean that the source code is available but that it is allowed to be freely modified and shared regardless of whether the users work in academia or commercial settings (although it should be noted that practical limitations are in place for most 'free software' licenses (e.g., the GNU GPL license), as these require any software that re-uses the GNU GPL-licensed code to be licensed under GNU GPL as well, typically a non-starter for commercial companies that usually have some form of proprietary code as well).
- Open-source software does not imply lack of professional quality. Many open-source projects are maintained by dedicated teams with robust testing and good programming practices. In genomics, projects like samtools (https://github.com/samtools/samtools) [19505943], an MIT-licensed (https://opensource.org/license/mit-0) project with over 80 contributors and 50,000+ citations, and the Genome Analysis Toolkit (GATK, https://github.com/broadinstitute/gatk) [20644199], now open-source with over 100 maintainers and 26,000+ citations, exemplify this standard. In proteomics, Percolator (https://github.com/percolator/percolator) [17952086] has over 700 citations and 20 contributors, serving as a core tool for projects like MS2Rescore [35803561], OpenMS [38366242], and Crux [36598107]. Other successful open-source projects in proteomics, such as OpenMS [38366242], Skyline [20147306], PeptideShaker [25574629], ThermoRAWFileParser [31755270], and ProteoWizard [23051804], demonstrate the benefits of transparency and collaboration. Despite these successes, academic open-source proteomics software is still perceived as lower quality. In 2018, Rob Smith highlighted the community’s concerns about academic proteomics and metabolomics software, including poor documentation, lack of transparency, and limited support [29546988], though it should be noted that much of this feedback was directed at academic and free-for-academic-use software, rather than exclusively open-source software.
...
- **Alterations Post-Publication**: Software is deposited in GitHub as open-source during the submission of the manuscript, but after publication, software licenses in GitHub repositories are changed, or repositories are deleted or made private, all of which complicates efforts to ensure long-term accessibility. This practice is particularly egregious, and can be legally frought as certain licenses, such as the GNU GPL, require explicit, written permission of every single contributor to the code before the license can be changed.
- **License Misuse or Ambiguity**: Some repositories may use inappropriate or ambiguous licenses, causing confusion about the terms of use, distribution, and modification (this is discussed in more detail in the section Licenses in proteomics software).
- **Obscure Dependencies**: Software repositories may have dependencies that are not clearly documented, which may require closed-source or proprietary software for the software to compile and/or run. This can create barriers for other researchers attempting to run or build on this software, as they may not have access to these necessary components, or may need to purchase expensive licenses to obtain this access. Clear documentation of all dependencies along with their licensing terms is essential to ensure transparency and reproducibility. Note that many OSS licenses require the license to travel with the code, forcing anyone implementing the code to be explicit about that code's license. 

## Licenses in proteomics software

We want to emphasize a fundamental aspect in any software development: the choice of the licenses. Licenses serve as the foundation for defining key aspects of software, including commercialization, code reuse, distribution, and proper citation, and, legally, the absence of a license imlpies maximal restriction. In other words, software without a license is as closed as it can be! It is therefore crucial to provide a license, and vital to choose a relevant one.

As the gold standard for proteomics software development, we recommend to use a standard OSS license, with a primary focus on the most tried-and-true licenses: Apache 2.0, MIT, BSD, LGPL, and GPL; the full list of applicable licenses can be found at (https://opensource.org/licenses).
The abovementioned licenses are all well known, are in use across many fields, and are well understood by the academic community and by industry.
Additionally, they are compatible with the FAIR principles and the Open Source Initiative (OSI) guidelines [36241754].
These established licenses moreover all have a clear, legally sound definition of what is allowed and what is not, and how the software can be distributed, reused, and cited.

Somewhat shockingly, many proteomics code repositories do not have a software license specified at all, in addition to many that use a variety of OSS and non-OSS licenses being used (**Figure 1**).
It is important to once again note that without a specified license the software is the exact opposite of open, as in this case the software and contributions are exclusively owned by the authors, and no one can use, copy, or distribute the contributions.
Given the largest category of source code for proteomics tools have unspecified licenses thus suggests a fundemantal misunderstanding of software licensing, as the source code has been put online, likely with the best of intentions.
...
This is particularly vital in proteomics, where bioinformatics is integral to every workflow, and progress depends on the synergy between wet-lab experimentation and computational innovation. The ability to extend and build on top of existing algorithms is therefore crucial for scientific progress.

Indeed, the field of proteomics has already greatly benefited from this open-source approach.
...
This complete feedback loop is unique to OSS, where contributions from the community enhance the quality and precision of software over time.  Compared to proprietary software, OSS can often move faster and defray development costs by enabling users to build and contribute the features they need, rather than hoping that the maintainers of the software are willing or able to add the features themselves. This dynamic frees developers from the burden of predicting and implementing every possible use case and shifts some of the burden of innovation to the broader community. For users, OSS reduces reliance on software maintainers, allowing research to advance even in the absence of formal support.
...
OSS can thus foster a culture of shared accountability, where code is not just released but continuously scrutinized and refined, driving the field forward in a collective effort toward scientific rigor.
We have indeed observed this in our own projects: at the time of writing, quantms [38965444] and mokapot [33596079] now have twelve and thirteen contributors, respectively.

## Open source and ML/AI models in proteomics

Machine learning and deep learning are increasingly used in proteomics, with examples like the MS2 prediction model MS2PIP [37140039] and the peptide _de novo_ sequencing models Casanovo [39080256] and InstaNovo [https://www.biorxiv.org/content/10.1101/2023.08.30.555055v3]. Many deep learning-based proteomics tools enhance reproducibility by clearly reporting source code, training parameters, and other details. While closed-source tools have also contributed to research, their models may carry bias, and their potential usefulness can be hard to assess when code and models are not accessible to others. It should here be noted that it can be perceived as contentious that some closed-source or commercial models are trained on publicly shared community datasets, which are themselves typically shared under highly permissive open access or public domain licenses.

Indeed, similar to how open-source software has proven its value by removing barriers to learning, sharing, and improving systems, AI models in proteomics should come with similar freedoms: autonomy, transparency, ease of reuse, and collaborative improvement. The Open Source Initiative's Open Source AI Definition (OSAID) outlines these freedoms, stating that users should be able to:
...
AI and machine learning are more than software; they encompass data, configurations, documentation, and new artifacts like model weights and biases. Open source concepts should apply to this entire system, including the models, parameters, and structural elements. However, it is currently remains unclear what mechanisms or licenses ensure that these models, particularly their parameters, are made freely available for use, research, modification, and sharing. We therefore recommend clear assertions accompanying parameter distribution to ensure that these remain freely accessible.

## Increasing emphasis on open science and open source by funding agencies

As open science gains prominence, major funding agencies worldwide are implementing mandates to ensure that software developed with public funds is made openly accessible and reusable.
Horizon Europe, the European Commission's flagship research program, has set amnbitious goals for open science, strongly promoting that research outputs, including software, are shared under open or free licenses aligned with FAIR principles (https://commission.europa.eu/about-european-commission/departments-and-executive-agencies/digital-services/open-source-software-strategy_en).
Additionally, all Horizon Europe funded research is required to establish a data management plan (DMP), which is a structured document that outlines plans for, amongst others, open software and code sharing, including tools needed for interoperability.
...
These challenges can deter long-term commitment, with some researchers transitioning to closed-source software after facing sustainability issues.
...
- **Potential Solutions**:
  - **Recognition for OSS Contributions**: Institutions and funding agencies should acknowledge OSS maintenance as valuable scholarly work, similar to publications, and include it in grant and tenure evaluations, as is, for instance, the case in thh European Commission's ERC programme CV template.
...
- **Potential Solutions**:
  ...
  - **Transition Plans**: Projects should develop clear transition plans, ensuring that new maintainers can seamlessly take over.
    This could involve thorough documentation, onboarding guidelines, and mentoring new contributors. Allowing (part-time) funding for developer positions as part of grants will also help establish longer-term positions for dedicated software developers and maintainers.

Addressing these challenges clearly requires a multi-pronged approach, combining changes in funding structures, academic incentives, and community engagement.
The scientific community, funding agencies, companies, and academic institutions must therefore collaborate to ensure that OSS can continue to thrive, and to bring about the ambitious vision towards open source academic software as proposed by several funding agencies.
By addressing these challenges head-on, we can thus build a more sustainable and collaborative ecosystem for open-source scientific software, ultimately driving innovation and reproducibility in proteomics research.
...
A primary purpose of the academic laboratory is the training of graduate and post-doctoral students. These positions are, by nature, of limited duration. The creation and development of software tools can be an ideal mechanism for creating a deep understanding of the concepts and best practice of proteomics. However, tools created during training can languish following the graduation and departure of the student unless there is a considered and established plan for sustainability in place.

We propose a tried and true procedure for sustainable software using the following established practices. 
...
In fact, we would argue that healthy OSS projects must be financially supported by methods such as charitable means, grants, or commercialization for the project to be sustainable.
We discuss a few commercialization models that have become popular with OSS, which try to strike a balance between supporting openness while safeguarding future development.
It is worth noting that these strategies are not necessarily mutually exclusive.

**Dual licensing.**
A popular commercialization option for OSS has been to offer the software under both a strong free software, so-called 'copyleft' license (like GPL or AGPL), which is usually incompatible with commercial use, as well as a more permissive commercial license. The code itself is typically the same for both license types. The difference lies in how the code can be used, modified, and redistributed depending on the license under which it is acquired.
Projects using this strategy ensure free availability of the code through the strong copyleft license (GPL, AGPL, etc.), but, as these licenses require any derivative works to be published under a similar open-source license, this license is often anathema to corporate users.
Thus, these projects also offer a more permissive commercial license to paying customers, allowing them to use the OSS project within proprietary code without copyleft restrictions.
...
RStudio is currently available under an open-source AGPLv3 license, or under a commercial license.
Notably, for this strategy, developers should make sure to include a "contributor license agreement" as part of their requirements for new contributors to ensure their contributions can be distributed under both licenses.

**Support or services.**
Some OSS projects commercialize by offering support services or new feature development at a cost.
Indeed, users, particularly from commercial entities, are often willing to pay for specialized training and ongoing, dedicated support for their use of OSS projects.
In specific instances, it may even be the case that outside entities are able to pay for the prioritization of specific features. For example, the major mass spectrometry instrument vendors have been providing financial support to both the Skyline and Proteowizard projects to ensure features, support, and documentation are provided for their customers.
This path must be tread carefully though; while there is benefit to allowing sponsored features, and they do tend to benefit all users once implemented (vendor-added features in Skyline are available to all other vendors with compatible instrumentation), such a model risks losing control over the direction of an OSS project. 
Red Hat is the most prominent example of a company that uses this strategy to commercialize their enterprise Linux offering. 

**Software as a service (SaaS).**
The SaaS commercialization model has become increasingly popular in recent times.
When using a SaaS model, the OSS project remains open-source, but commercialization occurs by building a user-friendly, often online platform around it to allow users to easily interact with the software.
...
**Open-core.**
The open-core commercialization model provides access to new, so-called value-added features only to paying customers. While essential functionality is freely provided, optional but useful features such as a nicer user interface or early access to new features are paid-for.
Some variants of this model use a time delay for new features, where paying users receive early access to new features, while users of the fully OSS version have to wait a while.
...
As proteomics research increasingly relies upon computational tools, embracing open-source and FAIR principles is essential to ensure transparency, reproducibility, and accessibility.
We urge researchers, funding agencies, institutions, and companies to prioritize open-source and FAIR practices, especially in publicly funded work, to create a truly collaborative scientific ecosystem, while simultaneously allowing commercialization efforts.
...
| Lennart Martens   | lennart.martens@ugent.be       | VIB—UGent | 0000-0003-4277-658X |
